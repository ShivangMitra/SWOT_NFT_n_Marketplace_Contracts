...
contract verifySellSignature {
    function getMessageHashFixSell(
    ...
    function getMessageHashAuctionSell(
        address _nftContract,
        uint256 _tokenId,
        uint256 _SellerPrice,
        uint256 _timeOfOrder,
        uint256 _auctionDuraton
    ) public pure returns (bytes32) {
        return
            keccak256(
                abi.encodePacked(
    ...
    function verifyFixSell(
        ...
        bytes32 messageHash = getMessageHashFixSell(
            _NFTcontract,
            _tokenId,
            _price,
            _timeOfCreation
    ...
    function verifyAuctionSell(
        ...
        bytes32 messageHash = getMessageHashAuctionSell(
            _nftContract,
            _tokenId,
            _SellerPrice,
            _timeOfOrder,
...
contract SwotMarketPlace is Ownable, ReentrancyGuard, verifySellSignature {
    ...
    function SetMarketPlaceFeesPercentage(uint256 _feesPercentage)
        public
        onlyOwner
        ...
        )
    {
        ...
        (royaltyOwner, royaltyAmount) = Inft(NFTcreate).royaltyInfo(
            _tokenId,
            _price
        );
    ...
    function buyAtFixedPrice(
        ...
        require(
            !fixPriceSignatureChecking[_signature],
            "Already sold using this signature"
        );
        ...
        require(
            ...
            "Signature doesnot match with the signer"
        );
        (
            uint256 priceTobePaid,
            uint256 royaltyAmount,
            address royaltyOwner,
            uint256 platformCharge
        ) = priceOfNft(_nftContract, _tokenId, _price);
    ...
    function currentHighestPrice(address _nftContract, uint256 _tokenId)
        public
        view
        returns (address buyer, uint256 price)
    {
        require(NFTcreates[_nftContract], "No contract exist");
        return (
            bids[_nftContract][_tokenId].buyer,
            bids[_nftContract][_tokenId].endingPrice
        );
    }

    function CancelAuctionPriceNFTtSell(
        address _nftContract,
        uint256 _tokenId,
        uint256 _SellerPrice,
        uint256 _timeOfOrder,
        ...
        require(NFTcreates[_nftContract], "No contract exist");
        require(
            !AuctionPriceSignatureChecking[_signature],
            "Already sold using this signature"
        );
        ...
        require(
            block.timestamp <= _timeOfOrder + _auctionDuraton,
            "Auction already ended"
        );
        AuctionPriceSell storage details = bids[_nftContract][_tokenId];
        if (details.endingPrice != 0) {
            uint256 amount = details.endingPrice;
            (uint256 priceTobePaid, , , ) = priceOfNft(
                _nftContract,
                _tokenId,
                amount
            );
    ...
    function buyAtAuctionPrice(
        ...
        if (details.endingPrice != 0) {
            (uint256 amountReturned, , , ) = priceOfNft(
                _nftContract,
                _tokenId,
                details.endingPrice
            );
    ...
    function auctionEnd(
        ...
        if (details.endingPrice > 0) {
            (
                ...
                address royaltyOwner,
                uint256 platformCharge
            ) = priceOfNft(_nftContract, _tokenId, details.endingPrice);
            ...
            Inft(_nftContract).safeTransferFrom(
                _owner,
                details.buyer,
                _tokenId,
                ""
